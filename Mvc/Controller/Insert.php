...
if(method_exists($PageComponents,$Model_Get)){
    ...
    $PageHtml = $PageComponents->$Model_Get($CreateLink,$Convert);
    
    $Folder->CreatFolder($CreateLink,$PageHtml);
    unset($Convert);
}
if(!empty($_FILES['Img'])){
    $Folder->ImageUpload($_FILES['Img'],$CreateLink);
}

if($Crud->Insert($_POST,$Model) == true){
    echo $Status->Success();
}
else {
    echo $Status->Failed();
